 ...
 *
 *   소형(<3천억) 25%     하루 15건   잠김 33%   24.5%에 사서 익일 시가 +0.71%p 상회 42%
 ...
 *
 * **2026-09-26, 사용자가 문턱을 올려 달라고 해서 다시 쟀습니다**
 * (27거래일 792건, `scripts/measure-sangtta-threshold.mjs`). 세 규모 중 올릴 수
 * 있는 것은 소형 하나였습니다.
 *
 *          문턱  하루건수  잠김률  사각지대
 *   소형    24%   16.5건    30%     18%
 *           25%   14.6건    33%     19%   ← 사각지대 거의 그대로, 확률만 오름
 *           26%   12.3건    36%     27%      여기서부터 값을 치름
 *   중형    25%    1.5건    41%     19%   ← 그대로 둠
 *           26%    1.3건    40%     33%      확률이 오히려 내려감
 *           27%    0.7건    50%     52%      절반을 못 봄
 *   대형    27%    0.4건    40%      0%   ← 그대로 둠
 *           28%    0.2건    40%     50%      확률은 같고 절반을 잃음
 *
 * 그래서 **소형만 24 → 25로 올렸습니다.** 중형 26은 확률이 41→40%로 내려가면서
 * 사각지대만 늘어 어느 쪽으로도 이득이 없고, 대형 28은 확률이 그대로인 채 잠김
 * 넷 중 둘을 놓칩니다. 올려 달라는 요청이었지만 두 곳은 올리면 나빠집니다.
 *
 * 올린 대가는 진입가입니다. 24.5%에 사서 +0.71%p를 낸 자리가 25.5% 근처로 밀리므로
 * 잠김 확률이 올라도 한 건당 초과수익은 줄 수 있습니다. 잠김률과 수익률은 다른
 * 숫자이고, 여기서 잰 것은 앞의 것뿐입니다 -- 진입가까지는 [[sangtta-verdict]]를
 * 다음 채점 때 다시 봐야 합니다.
 *
 * 하루 건수(16.5 → 14.6)는 **알림 수가 아닙니다.** 근거 없는 것은 안 보내므로 실제로
 * 오는 통은 이보다 훨씬 적습니다. 문턱을 올려도 알림이 확 줄지 않는 이유입니다.
 *
...
// 알림 문턱(소형 25%)보다 낮게 둡니다 -- 문턱 아래 구간도 찍어 둬야 다음에 문턱을
// 다시 잴 때 비교할 것이 남습니다. 알림과 수집을 같이 올리면 근거가 사라집니다.
